...
void main()
{
    vec3 Kd = vec3(1.0, 1.0, 1.0); // Coeficiente de reflexión difusa
    vec3 Ld = vec3(2.0, 2.0, 2.0); // Intensidad de la luz
    vec3 ambient = 0.2 * Ld; // Componente ambiental
    ...
    vec4 texel = texture(sampler_tex, frag_texcoord) * vec4(ambient + diffuse + specular, 1.0);
    if (texel.a < 0.5)
        discard;
    outColor = texel;
}
